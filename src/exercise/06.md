...
- Handle form submits and changes
- Use a ref (React.useRef) to get the value from the input event
- Use the useState hook to show an error message when the user type invalid
  input
- Use the useState hook along with the value attribute on the input element to
  not even let the user type invalid input
